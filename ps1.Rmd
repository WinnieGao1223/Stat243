...
function locweather(){
  ...
  for c in {2015..2018}; do
    ...
    if [$3 == "January"] 
      then grep "${c}01" tmp.csv > tmp2.csv 
    fi
    if [$3 == "Feburary"] 
      then grep "${c}02" tmp.csv > tmp2.csv 
    fi
    if [$3 == "March"] 
      then grep "${c}03" tmp.csv > tmp2.csv 
    fi
    if [$3 == "April"] 
      then grep "${c}04" tmp.csv > tmp2.csv
    fi
    if [$3 == "May"] 
      then grep "${c}05" tmp.csv > tmp2.csv
    fi
    if [$3 == "June"] 
      then grep "${c}06" tmp.csv > tmp2.csv
    fi
    if [$3 == "July"] 
      then grep "${c}07" tmp.csv > tmp2.csv
    fi
    if [$3 == "August"] 
      then grep "${c}08" tmp.csv > tmp2.csv
    fi
    if [$3 == "September"] 
      then grep "${c}09" tmp.csv > tmp2.csv
    fi
    if [$3 == "October"] 
      then grep "${c}10" tmp.csv > tmp2.csv
    fi
    if [$3 == "November"] 
      then grep "${c}11" tmp.csv > tmp2.csv
    fi
    if [$3 == "December"] 
      then grep "${c}12" tmp.csv > tmp2.csv
    fi
...
#locweather "DEATH VALLEY" TMAX March
...
curl -s "https://www1.ncdc.noaa.gov/pub/data/ghcn/daily/"|sed -n 's/.*href="\([^"]*\).*/\1/p'|grep "txt" >filenames.csv
...
library(reticulate)
#use_python("/Users/Winnie/anaconda3/bin/python")
#py_available(TRUE)
a = list(1,2,3,4,5)
#testfile = read.csv("DeathValleyMarchTMAX.csv",header = FALSE, sep = ",")
